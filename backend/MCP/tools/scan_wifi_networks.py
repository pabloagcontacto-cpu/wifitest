...
def read_airodump_csv_text(csv_path: Path, max_chars: int = RAW_CSV_MAX_CHARS) -> str:
    """Read a bounded, decode-tolerant preview of an airodump-ng CSV file."""
    with csv_path.open("r", encoding="utf-8", errors="replace", newline="") as csv_file:
        raw_text = csv_file.read(max_chars + 1)
    ...
    return (
        raw_text[:max_chars]
        + "\n\n--- WIFITEST: raw CSV truncated; normalized data contains the parsed result. ---"
    )
